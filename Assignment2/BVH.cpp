...
void BVH::quaternionMoveJoint(JOINT* joint, float* mdata, float scale)
{
    ...
    {        		
		...
		switch(channel->type){
			case X_POSITION:
				joint->transform.translation += glm::vec3(value*scale, 0, 0);	
				break;

			case Y_POSITION:
				joint->transform.translation += glm::vec3(0, value*scale, 0);      			
				break;

			case Z_POSITION:
				joint->transform.translation += glm::vec3(0, 0, value*scale);
				break;
    ...
    if (joint->parent != NULL) {
        joint->transform.translation = joint->parent->transform.quaternion * joint->transform.translation + joint->parent->transform.translation;
